...
template <typename T>
void project::VendingMachine<T>::update_product(const T &productCode, double newPrice, int newStock) {
    for (auto &p : products)
        if(p.id == productCode)
        {
            p.price=newPrice;
...
template<typename T>
void project::VendingMachine<T>::dispensing(const T &productCode) {
